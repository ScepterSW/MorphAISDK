...
int PostProcessVzenseBox::Process(MODEL_INFO *model_info, float conf_threshold, float nms_threshold, detect_result_group_t *od_results)
{
...
int PostProcessVzenseBox::process_u8(uint8_t *input, int *anchor, int grid_h, int grid_w, int height, int width, int stride,
                   ...
                   float threshold, uint32_t zp, float scale)
{
    ...
    for (int a = 0; a < 3; a++)
    {
        for (int i = 0; i < grid_h; i++)
        {
            ...
            {
                uint8_t box_confidence = input[((5 + 1) * a + 4) * grid_len + i * grid_w + j];
                if (box_confidence >= thres_u8)
                {
                    int offset = ((5 + 1) * a) * grid_len + i * grid_w + j;
